...
DISPOSABLE_DIRECTORIES = {
    "outgoing_uploads": "temporary copies staged for sidebar test sends",
...
_STORAGE_COMPONENTS = [
    ...
    },
    {
        ...
        "clear_history": "reset",
        "migration_action": "history reset removes all diagnostics; rerun probes on the current machine (cleanup-artifacts alone retains the latest native probe)",
    },
]
...
def build_artifact_cleanup_report(data_dir: str | Path = "data", apply: bool = False) -> dict[str, Any]:
    data_root = _safe_cleanup_data_root(data_dir)
    if apply and data_root.is_dir():
        with history_writer_fence(data_root, label="artifact_cleanup"):
            return _build_artifact_cleanup_report(data_root, apply=True)
    return _build_artifact_cleanup_report(data_root, apply=False)


def _build_artifact_cleanup_report(data_root: Path, *, apply: bool) -> dict[str, Any]:
    candidates: list[dict[str, Any]] = []
    missing: list[dict[str, str]] = []
    retained: list[dict[str, Any]] = []
    total_reclaimable = 0
    deleted_count = 0
    browser_profile_blockers: list[dict[str, Any]] | None = None

    # Validate the complete cleanup boundary before the first deletion. Paths
    # stay lexical so a symlink/junction can never be resolved into a retained
    # in-root target or an external directory.
    artifact_paths = {
        relative: _safe_data_child(data_root, relative)
    ...
    retained_paths = {
        relative: _safe_data_child(data_root, relative)
        for relative in _RETAINED_ARTIFACTS
    }
    native_cleanup = _native_diagnostics_cleanup(data_root)
    file_workspace = _safe_data_child(data_root, "file_workspace")
    preflight_paths = [
        *artifact_paths.values(),
        *directory_paths.values(),
        *retained_paths.values(),
        *(path for path, _reason in native_cleanup["candidates"]),
        *(path for path, _reason in native_cleanup["retained"]),
        file_workspace,
    ]
    for path in preflight_paths:
        _validate_cleanup_path(path)

    for relative, reason in sorted(DISPOSABLE_ARTIFACTS.items()):
    ...
    for relative, reason in sorted(DISPOSABLE_DIRECTORIES.items()):
        path = directory_paths[relative]
        if not path.exists():
            missing.append({"relative_path": relative, "reason": reason})
            continue
        item = _artifact_item(data_root, path, reason)
        if relative == "runtime/sidebar_browser_profile":
            if browser_profile_blockers is None:
                browser_profile_blockers = sidebar_browser_runtime_blockers(data_root)
                browser_profile_blockers.extend(
                    lease
                    for lease in active_history_writer_leases(data_root)
                    if str(lease.get("label") or "") == "sidebar_browser_lifecycle"
                )
            if browser_profile_blockers:
                item["action"] = "retain_active"
                item["blockers"] = browser_profile_blockers
...
def _safe_cleanup_data_root(data_dir: str | Path) -> Path:
    lexical = Path(os.path.abspath(os.fspath(data_dir)))
    root_stat = _safe_lstat(lexical)
    if root_stat is not None:
        if _is_reparse_point(root_stat) or not stat.S_ISDIR(root_stat.st_mode):
            raise PermissionError(f"unsafe cleanup data directory: {lexical}")
    try:
        canonical = lexical.resolve(strict=False)
    except OSError as exc:
        raise PermissionError(f"cleanup data directory cannot be resolved: {lexical}") from exc
    if canonical != lexical:
        raise PermissionError(f"cleanup data directory must use its canonical path: {lexical}")
    return lexical
...
def _safe_data_child(data_root: Path, relative: str) -> Path:
    ...
    if (
        ...
        raise PermissionError(f"invalid data child path: {relative}")
    candidate = data_root.joinpath(*relative_path.parts)
    try:
        candidate.relative_to(data_root)
    except ValueError as exc:
        raise PermissionError(f"path outside data directory: {candidate}") from exc
    for index in range(1, len(relative_path.parts) + 1):
        component = data_root.joinpath(*relative_path.parts[:index])
        component_stat = _safe_lstat(component)
        if component_stat is None:
            continue
        if _is_reparse_point(component_stat):
            raise PermissionError(f"cleanup path uses a symlink or reparse point: {candidate}")
        if index < len(relative_path.parts) and not stat.S_ISDIR(component_stat.st_mode):
            raise PermissionError(f"cleanup path parent is not a directory: {candidate}")
    try:
        canonical = candidate.resolve(strict=False)
    except OSError as exc:
        raise PermissionError(f"cleanup path cannot be resolved: {candidate}") from exc
    if canonical != candidate:
        raise PermissionError(f"cleanup path is not canonical: {candidate}")
    return candidate


def _native_diagnostics_cleanup(data_root: Path) -> dict[str, list[tuple[Path, str]]]:
    native_dir = _safe_data_child(data_root, "native_diagnostics")
    native_stat = _safe_lstat(native_dir)
    if native_stat is None:
        return {"candidates": [], "retained": []}
    if not stat.S_ISDIR(native_stat.st_mode):
        raise PermissionError(f"native diagnostics path is not a directory: {native_dir}")
    files = _walk_private_cleanup_tree(native_dir)
    keep: set[Path] = set()
    ...
    snapshots = sorted(
        (
            path
            for path in files
            if path.parent == native_dir
            and path.match("native-migration-*.json")
            and path.name != "native-migration-latest.json"
        ),
...
def _delete_candidate(path: Path) -> tuple[bool, str]:
    try:
        ...
        if _is_reparse_point(path_stat):
            raise PermissionError(f"refusing to delete cleanup reparse point: {path}")
        if stat.S_ISREG(path_stat.st_mode):
            path.unlink()
        elif stat.S_ISDIR(path_stat.st_mode):
            _walk_private_cleanup_tree(path)
            _delete_private_cleanup_tree(path)
        else:
            return False, "not_file_or_directory"
        return _safe_lstat(path) is None, ""
    except Exception as exc:
...
def _tree_size(path: Path) -> int:
    ...
    if path_stat is None:
        return 0
    if _is_reparse_point(path_stat):
        raise PermissionError(f"refusing to size cleanup reparse point: {path}")
    if stat.S_ISREG(path_stat.st_mode):
        return int(path_stat.st_size)
    if not stat.S_ISDIR(path_stat.st_mode):
        raise PermissionError(f"unsupported cleanup path type: {path}")
    return sum(int(child.lstat().st_size) for child in _walk_private_cleanup_tree(path))


def _validate_cleanup_path(path: Path) -> None:
    path_stat = _safe_lstat(path)
    if path_stat is None:
        return
    if _is_reparse_point(path_stat):
        raise PermissionError(f"cleanup path uses a symlink or reparse point: {path}")
...
def _walk_private_cleanup_tree(root: Path) -> list[Path]:
    files: list[Path] = []
    pending = [root]
    while pending:
        current = pending.pop()
        ...
        if _is_reparse_point(current_stat):
            raise PermissionError(f"cleanup tree uses a symlink or reparse point: {current}")
        if not stat.S_ISDIR(current_stat.st_mode):
            raise PermissionError(f"cleanup tree path is not a directory: {current}")
        ...
        for child in children:
            ...
            if _is_reparse_point(child_stat):
                raise PermissionError(f"cleanup tree uses a symlink or reparse point: {child}")
            if stat.S_ISDIR(child_stat.st_mode):
                pending.append(child)
            elif stat.S_ISREG(child_stat.st_mode):
                files.append(child)
            else:
                raise PermissionError(f"unsupported cleanup tree entry: {child}")
    return sorted(files, key=lambda item: item.as_posix())


def _delete_private_cleanup_tree(root: Path) -> None:
    root_stat = _safe_lstat(root)
    if root_stat is None:
        return
    if _is_reparse_point(root_stat) or not stat.S_ISDIR(root_stat.st_mode):
        raise PermissionError(f"cleanup tree changed identity before deletion: {root}")
    ...
    for child in children:
        ...
        if child_stat is None:
            continue
        if _is_reparse_point(child_stat):
            raise PermissionError(f"cleanup tree changed to a symlink or reparse point: {child}")
        if stat.S_ISDIR(child_stat.st_mode):
            _delete_private_cleanup_tree(child)
        elif stat.S_ISREG(child_stat.st_mode):
            child.unlink()
        else:
            raise PermissionError(f"unsupported cleanup tree entry: {child}")
